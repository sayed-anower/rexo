...
// Popular regions: detected timezone shown first, then global defaults.
...
interface FormState {
  ...
  extra_vars: Record<string, string>; // per-automation values for user-added [my_var] tokens
}
...
export function AutomationPage({
  ...
  // Unknown template variables awaiting values for THIS automation only.
  ...
  const openCreate = (kind: ScheduleKind) => {
    if (eligibleInvoices.length === 0) {
      onToast('No unpaid invoices available. Add invoices first before creating automations.');
      return;
    }
    setEditingId(null);
    setPendingVars(null);
  ...
  const openEdit = (s: AutomationSchedule) => {
    ...
    setForm({
      ...
      timezone: s.timezone || detectBrowserTimezone(),
      ...
      intervalValue: s.interval_minutes
        ...
        ? 30
        : 1,
      extra_vars: s.extra_vars ? { ...s.extra_vars } : {},
    });
    setFormOpen(true);
  };
  ...
  const handleSave = async () => {
    if (!canSave) return;
    // Custom variables in the selected template (e.g. [my_var]): ask once for
    // their values and store them on THIS automation. Saved automations keep
    // their values — they are only re-shown pre-filled when editing.
    if (form.kind === 'automation') {
      const tmpl = templates.find((t) => t.id === form.template_id);
      const unknown = findUnknownVars(tmpl?.subject, tmpl?.body);
  ...
  const doSave = async (extraVarsOverride?: Record<string, string>) => {
    if (!canSave) return;
    setSaving(true);
    try {
      const extraVars = extraVarsOverride ?? form.extra_vars ?? {};
          ...
          : {
              ...
              interval_minutes:
                form.frequency === 'minutely'
                  ...
                  ? Math.max(1, Math.round(form.intervalValue || 1)) * 60
                  : undefined,
              extra_vars: Object.keys(extraVars).length ? extraVars : {},
            };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-white dark:bg-surface border border-line dark:border-line shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <button
            ...
            disabled={(tab === 'automation' && templates.length === 0) || eligibleInvoices.length === 0}
          ...
          <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-3xl bg-white dark:bg-surface border border-line dark:border-line p-6 sm:p-8 shadow-2xl space-y-5">
            ...
            {form.kind === 'automation' ? (
              <div>
                ...
                {(() => {
                  const tmpl = templates.find((t) => t.id === form.template_id);
                  const unknown = tmpl ? findUnknownVars(tmpl.subject, tmpl.body) : [];
                  if (!unknown.length) return null;
                  return (
                    <p className="text-[10px] mt-1.5 flex items-start gap-1.5 text-amber-600 dark:text-amber-400">
                      <Info className="w-3 h-3 shrink-0 mt-0.5" />
                      This template uses {unknown.length} custom variable{unknown.length === 1 ? '' : 's'} (
                      {unknown.map((v) => `[${v}]`).join(', ')}) — you&apos;ll set their value for this automation before
                      saving.
                    </p>
                  );
                })()}
      ...
      {/* Per-automation custom variable values ([my_var] -> value) */}
      {formOpen && pendingVars && pendingVars.length > 0 && (
        <VariableValuesModal
          vars={pendingVars}
          initialValues={form.extra_vars}
          onCancel={() => setPendingVars(null)}
          onConfirm={(values) => {
            setForm((f) => ({ ...f, extra_vars: values }));
            setPendingVars(null);
            doSave(values);
          }}
        />
