...
const TEMPLATE_CHANNEL_META: { id: ChannelType; label: string; icon: typeof Mail }[] = [
...
interface CustomEmailTemplatesProps {
  ...
  onSendCustomEmail: (template: CustomEmailTemplate, invoice: Invoice, extraVars?: Record<string, string>, channel?: ChannelType) => Promise<void>;
...
export function CustomEmailTemplates({
  ...
  // Which channel this template is being sent through right now.
  const [sendChannel, setSendChannel] = useState<ChannelType>('email');
  ...
  const handleOpenEdit = (tmpl: CustomEmailTemplate) => {
    setEditingTemplate({ ...tmpl, channels: tmpl.channels && tmpl.channels.length ? tmpl.channels : ['email'] });
    setIsEditorOpen(true);
  };
  ...
  const handleOpenSendModal = (tmpl: CustomEmailTemplate) => {
    ...
    setPendingVars(null);
    setExtraVarValues({});
    setSendChannel(((tmpl.channels && tmpl.channels.length ? tmpl.channels : ['email']) as ChannelType[])[0] || 'email');
  ...
  // Channels this template supports, for the invoice being targeted.
  const supportedChannels = (tmpl: CustomEmailTemplate | null): ChannelType[] =>
    tmpl ? (tmpl.channels && tmpl.channels.length ? tmpl.channels : ['email']) : [];

  const channelAvailable = (inv: Invoice | undefined, ch: ChannelType): boolean => {
    if (!inv) return false;
    if (ch === 'email') return Boolean(inv.client_email);
    return Boolean(inv.client_phone);
  };
  ...
  const handleExecuteSend = async () => {
    ...
    await doSend(inv, extraVarValues, sendChannel);
  };
            ...
            >
              <div>
                ...
                <div className="flex flex-wrap items-center gap-1.5 mb-2">
                  {(tmpl.channels && tmpl.channels.length ? tmpl.channels : ['email']).map((ch) => {
                    const meta = TEMPLATE_CHANNEL_META.find((c) => c.id === ch);
                    if (!meta) return null;
                    const Icon = meta.icon;
                    return (
                      <span key={ch} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wider bg-main dark:bg-surface2 border border-line dark:border-line text-ink2 dark:text-ink2">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-ink dark:text-ink2 mb-1.5">
                  Send Via Channels <span className="text-[10px] font-semibold text-ink3">(pick one or more)</span>
                </label>
                <div className="grid grid-cols-3 gap-2">
                  {TEMPLATE_CHANNEL_META.map((ch) => {
                    const current: ChannelType[] =
                      (editingTemplate.channels && editingTemplate.channels.length ? editingTemplate.channels : ['email']) as ChannelType[];
                    const checked = current.includes(ch.id);
                    const Icon = ch.icon;
                    return (
                      <button
                        key={ch.id}
                        type="button"
                        onClick={() => {
                          const next = checked ? current.filter((c) => c !== ch.id) : [...current, ch.id];
                          setEditingTemplate({ ...editingTemplate, channels: next.length ? next : current });
                        }}
                ...
                <p className="text-[10px] text-ink3 mt-1">
                  WhatsApp &amp; SMS send the message body only — clean text with the payment link, no email signature.
                </p>
              </div>
              ...
              {((editingTemplate.channels || ['email']) as ChannelType[]).includes('email') && (
                <div>
                  <label className="block text-xs font-bold text-ink dark:text-ink2 mb-1">
                    Email Subject Line *
                  </label>
              ...
              <div>
                <label className="block text-xs font-bold text-ink dark:text-ink2 mb-1">
                  {((editingTemplate.channels || ['email']) as ChannelType[]).length > 1 || ((editingTemplate.channels || ['email']) as ChannelType[])[0] !== 'email'
                    ? 'Message Body * (used for every selected channel)'
                    : 'Email Message Body *'}
                </label>
                <textarea
                  required
          ...
          <div className="bg-white dark:bg-surface border border-line dark:border-line rounded-3xl max-w-xl w-full max-h-[80vh] overflow-y-auto p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {TEMPLATE_CHANNEL_META.map((ch) => {
                    const supported = supportedChannels(targetTemplate).includes(ch.id);
                    const available = supported && channelAvailable(selectedInvoice, ch.id);
                    const checked = sendChannel === ch.id;
                    const Icon = ch.icon;
                    return (
                      <button
                        key={ch.id}
                        type="button"
                        disabled={!available}
                        onClick={() => available && setSendChannel(ch.id)}
                        title={
                          !supported
                            ? `This template does not support ${ch.label}`
                            : !available
                            ? ch.id === 'email'
                              ? 'This invoice has no client email'
                              : 'This invoice has no client phone number'
                            : ch.label
                        }
                        className={`p-2.5 rounded-xl border text-center transition-all flex flex-col items-center gap-1 ${
                          checked && available
                      ...
                      >
                        <Icon className={`w-4 h-4 ${checked && available ? 'text-accent' : 'text-ink3'}`} />
                        <span className={`text-[11px] font-bold ${checked && available ? 'text-accent' : 'text-ink dark:text-ink2'}`}>
                          {ch.label}
                        </span>
                      </button>
                    );
                ...
                {sendChannel !== 'email' && (
                  <p className="text-[10px] text-ink3 mt-1">
                    Sent as a clean {sendChannel === 'whatsapp' ? 'WhatsApp' : 'SMS'} message with the payment link — no email signature is added.
                  </p>
                )}
              </div>
              ...
              {selectedInvoice && (
                <div className="p-4 rounded-2xl bg-main dark:bg-surface2/80 border border-line dark:border-line text-xs space-y-2">
                  <span className="text-[10px] font-extrabold text-ink3 uppercase tracking-wider block">
                    Live Rendered {sendChannel === 'email' ? 'Email' : sendChannel === 'whatsapp' ? 'WhatsApp' : 'SMS'} Preview
                  </span>
                  {sendChannel === 'email' && (
                ...
                >
                  {isSending ? (
                    ...
                    <>
                      ...
                      <span>Send via {sendChannel === 'email' ? 'Email' : sendChannel === 'whatsapp' ? 'WhatsApp' : 'SMS'}</span>
                    </>
                  )}
                </button>
              </div>
