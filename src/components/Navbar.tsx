    ...
    <header className="sticky top-0 z-50 border-b border-line dark:border-line bg-white/95 dark:bg-surface/95 backdrop-blur-md px-3 sm:px-6 py-2.5 transition-colors">
      <div className="flex items-center justify-between max-w-7xl mx-auto w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1">
          <button
            onClick={() => nav('/')}
            className="px-3 py-1.5 rounded-lg text-xs font-semibold text-ink2 dark:text-ink2 hover:text-ink dark:hover:text-white hover:bg-surface2 dark:hover:bg-surface2 transition-colors"
          >
            Home
          </button>
          <button
            ...
            <div className="relative">
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-60 rounded-2xl bg-white dark:bg-surface border border-line dark:border-line shadow-2xl py-2 z-[100] text-xs animate-in fade-in slide-in-from-top-2 duration-150">
                  ...
                  {/* Mobile Nav Links */}
                  <div className="lg:hidden border-b border-line dark:border-line py-1">
                    <button onClick={() => { setDropdownOpen(false); nav('/'); }} className="w-full text-left px-4 py-2 hover:bg-main dark:hover:bg-surface2 text-ink dark:text-ink2 flex items-center gap-2 font-medium">
                      <Home className="w-4 h-4 text-ink3" /> Home
                    </button>
                    <button onClick={() => { setDropdownOpen(false); nav('/about'); }} className="w-full text-left px-4 py-2 hover:bg-main dark:hover:bg-surface2 text-ink dark:text-ink2 flex items-center gap-2 font-medium">
                      <Info className="w-4 h-4 text-ink3" /> About
                    </button>
                    <button onClick={() => { setDropdownOpen(false); nav('/docs'); }} className="w-full text-left px-4 py-2 hover:bg-main dark:hover:bg-surface2 text-ink dark:text-ink2 flex items-center gap-2 font-medium">
                      <BookOpen className="w-4 h-4 text-ink3" /> Documentation
                    </button>
                    <button onClick={() => { setDropdownOpen(false); nav('/help'); }} className="w-full text-left px-4 py-2 hover:bg-main dark:hover:bg-surface2 text-ink dark:text-ink2 flex items-center gap-2 font-medium">
                      <LifeBuoy className="w-4 h-4 text-ink3" /> Help & Support
                    </button>
                    <button onClick={() => { setDropdownOpen(false); nav('/privacy'); }} className="w-full text-left px-4 py-2 hover:bg-main dark:hover:bg-surface2 text-ink dark:text-ink2 flex items-center gap-2 font-medium">
                      <Shield className="w-4 h-4 text-ink3" /> Privacy Policy
                    </button>
                  </div>
